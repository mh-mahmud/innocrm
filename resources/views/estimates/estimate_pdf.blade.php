                    ...
                    <td>
                        ...
                        <div class="text-right">
                            <table class="invoice-date-table">
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.estimate.estimate_date').': ' }}</td>
                                    <td>{{ !empty($estimate->estimate_date) ? Carbon\Carbon::parse($estimate->estimate_date)->translatedFormat('jS M, Y') : __('messages.common.n/a') }}</td>
                                </tr>
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.estimate.expiry_date').': ' }}</td>
                                    <td>{{ !empty($estimate->estimate_expiry_date) ? Carbon\Carbon::parse($estimate->estimate_expiry_date)->translatedFormat('jS M, Y') : __('messages.common.n/a') }}</td>
                                </tr>
                                @if(isset($estimate->sales_agent_id))
                                    <tr>
                                        <td class="font-weight-bold">{{ __('messages.invoice.sale_agent').': ' }}</td>
        ...
        <td colspan="2">
            <table width="100%" class="table table-bordered invoice-sales-items mt-2">
                ...
                <tr>
                    ...
                    <th scope="col" class="item-col">{{ __('messages.estimate.item') }}</th>
                    <th scope="col" class="text-right item-qty">{{ __('messages.estimate.qty') }}</th>
                    <th scope="col" class="text-right rate">{{ __('messages.estimate.rate') }}( <span
                                class="pdf-css">&#{{ getCurrencyIcon($estimate->currency) }}</span>
                        )
                    </th>
                    <th scope="col" class="item-taxes">{{ __('messages.estimate.taxes') }}</th>
