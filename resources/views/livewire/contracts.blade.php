<div>
    <div class="mt-0 mb-3 col-12 d-flex justify-content-end livewire-search">
        @if(!empty($customer))
            <div class="mt-2 m-r-0">
                {{Form::select('type', $typeArr, $filterContractType, ['id' => 'contractFilterType', 'class' => 'form-control status-filter', 'placeholder' => __('messages.placeholder.select_contract_type')]) }}
            </div>
        @endif
                        ...
                        <div class="d-flex contract-listing-description">
                            <div class="contract-data">
                                ...
                                <h3 class="contract-listing-title">
                                    <span data-toggle="tooltip" title="" data-original-title="{{ __('messages.common.customer') }}"><i
                                                class="fas fa-street-view fa_fa_20px"></i>&nbsp{{ html_entity_decode(Str::limit($contract->customer->company_name,15)) }}
                                    </span>
                                </h3>
                                <h3 class="contract-listing-title">
                                    <span data-toggle="tooltip" title="" data-original-title="{{ __('messages.common.contract_type') }}"><i
                                                class="fas fa-file-contract fa_fa_20px"></i>  &nbsp{{ html_entity_decode($contract->type->name) }}
                                    </span>
                                </h3>
                                @if(!empty($contract->contract_value))
                                    <h3 class="contract-listing-title">
                                        <span data-toggle="tooltip" title="" data-original-title="{{ __('messages.contract.contract_value') }}"><i
                                                    class="{{ getCurrencyClass(getCurrentCurrency()) }} fa_fa_20px"></i>  &nbsp{{ number_format($contract->contract_value, 2) }}
                                        </span>
                                    </h3>
                                @endif
                            </div>
        ...
        @empty
            <div class="mt-0 mb-5 col-12 d-flex justify-content-center mb-5 rounded">
                <div class="p-2">
                    ...
                    @else
                        <p class="text-dark">{{ __('messages.contract.no_contract_found') }}</p>
                    @endif
                </div>
            </div>
        @endforelse
