    ...
    <div class="row">
        ...
        @forelse($estimates as $estimate)
            <div class="col-12 col-md-6 col-xl-3 col-xlg-4">
                <div class="estimate-card card card-{{ \App\Models\Estimate::STATUS_COLOR[$estimate->status] }} shadow mb-5 rounded hover-card estimate-card-border height-210">
                        ...
                        <div class="d-flex w-100 justify-content-between">
                            <a href="{{ url('admin/estimates', $estimate->id) }}"
                               class="d-flex flex-wrap text-decoration-none">
                                <h4 class="invoice-clients invoice_title"> {{ Str::limit(html_entity_decode($estimate->title), 8, '...') }}</h4>
                        ...
                        <div class="d-table w-100">
                            <div>
                                <i class="fas fa-street-view"></i><span
                                        ...
                                        title="{{ $estimate->customer->company_name }}"> {{ Str::limit(html_entity_decode($estimate->customer->company_name), 15) }}</span>
                            </div>
                            ...
                            <span class="mt-mobile-2 badge-{{ App\Models\Estimate::STATUS_COLOR[$estimate->status] }} badge text-uppercase">{{  App\Models\Estimate::STATUS[$estimate->status] }}</span>
                        </span>
                            @if(!empty($estimate->estimate_expiry_date))
                                <span class="d-table-row text-nowrap w-100 {{ now() > Carbon\Carbon::parse($estimate->estimate_expiry_date)  ? 'text-danger' : '' }}">
