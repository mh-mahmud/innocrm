<div class="row">
    ...
    @forelse($members as $member)
        <div class="col-12 col-md-6 col-lg-4 col-xl-4 extra-large">
            <div class="livewire-card card {{ $loop->odd ? 'card-primary' : 'card-dark'}} shadow mb-5 rounded user-card-view user-card-mbl-height">
                    ...
                    @if($member->id != getLoggedInUserId())
                        <div class="mt-2 member-card-toggle card-toggle-mr">
                            <label class="custom-switch pl-0" data-placement="bottom"
                                   data-toggle="tooltip" title="{{ __('messages.common.status') }}">
                    ...
                    @if(empty($member->email_verified_at))
                        <div class="ml-auto mt-1 member-card-toggle">
                            <button class="btn btn-danger btn-sm p-0 pl-1 pr-1 email-verify-btn"
                                    data-id="{{ $member->id }}"
                                    data-toggle="tooltip" title="{{ __('messages.member.email_verify') }}">
